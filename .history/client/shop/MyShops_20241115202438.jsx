...
export default function MyShops() {
  ...
  useEffect(() => {
    ...
    // Ensure JWT is valid before making the API request
    if (jwt?.user?._id && jwt?.token) {
      listByOwner(
        { userId: jwt.user._id },
        { t: jwt.token },
        signal
        ...
        .then((data) => {
          if (data?.error) {
            ...
            setShops(data); // Ensure data is an array before setting state
          } else {
            console.error("Unexpected response format:", data);
            setShops([]); // In case of unexpected response, set shops to an empty array
          }
        })
        .catch((err) => {
      ...
      setRedirectToSignin(true); // Redirect if JWT is not valid
    }
  ...
  const removeShop = (shop) => {
    const updatedShops = shops.filter((s) => s._id !== shop._id);
    setShops(updatedShops);
  };
...
export default function MyShops() {
  ...
  useEffect(() => {
    ...
    // Ensure jwt is defined before making the API request
    if (jwt?.user?._id && jwt?.token) {
      listByOwner(
        {
          userId: jwt?.user?._id,
        ...
        .then((data) => {
          if (data && data.error) {
            ...
            setShops(data); // Ensure data is an array
          } else {
